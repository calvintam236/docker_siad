...
RUN apt-get update \
    ...
    && curl -L -O https://github.com/NebulousLabs/Sia/releases/download/v1.3.1/Sia-v1.3.1-linux-amd64.zip \
    && unzip Sia-v1.3.1-linux-amd64.zip \
    && rm Sia-v1.3.1-linux-amd64.zip \
    && mv Sia-v1.3.1-linux-amd64/siad /usr/local/bin/siad \
    && chmod a+x /usr/local/bin/siad \
    && rm -r Sia-v1.3.1-linux-amd64 \
